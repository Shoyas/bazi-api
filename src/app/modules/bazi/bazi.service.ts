import { Solar, Lunar, LunarYear, I18n } from 'lunar-typescript';
...
const calculateBazi = async (payload: IBaziRequest): Promise<IBaziResponseData> => {
  ...
  try {
    ...
    throw new AppError(400, 'Invalid timezone or time format');
  }
  ...
  const rawWuXingStats: Record<string, number> = {
    Wood: 0, Fire: 0, Earth: 0, Metal: 0, Water: 0,
  };
  
  const mapWuXing = (wx: string) => {
    if (wx.includes('木')) rawWuXingStats.Wood += wx.split('木').length - 1;
    if (wx.includes('火')) rawWuXingStats.Fire += wx.split('火').length - 1;
    if (wx.includes('土')) rawWuXingStats.Earth += wx.split('土').length - 1;
    if (wx.includes('金')) rawWuXingStats.Metal += wx.split('金').length - 1;
    if (wx.includes('水')) rawWuXingStats.Water += wx.split('水').length - 1;
  };
  ...
  const wuXingStats: Record<string, string> = {};
  ...
  const tenGodsDistribution: Record<string, string> = {};
